...
NPMLocation.prototype = {
  ...
  processPackageConfig: function(packageConfig, packageName) {
    ...
    if (packageConfig.peerDependencies) {
      for (d in packageConfig.peerDependencies)
        packageConfig.dependencies[d] = packageConfig.peerDependencies[d];
    }

    // warn if using jspm-style dependencies at this point
    for (var d in packageConfig.dependencies)
      if (!packageConfig.dependencies[d].match(/^(https?|git)[:+]/) && packageConfig.dependencies[d].indexOf(':') > 0)
        throw 'Package.json dependency %' + d + '% set to `' + packageConfig.dependencies[d] + '`, which is not a valid dependency format for npm.'
    ...
    // every npm package depends on github:jspm/nodelibs (a peer dependency in due course)
    if (!packageConfig.dependencies['nodelibs'])
      packageConfig.dependencies['nodelibs'] = 'github:jspm/nodelibs@^0.1.0';
